To save the app's transient data and to also access the database, you need to update the `ViewModel`s. Your `ViewModel`s interact with the database via the DAO and provide data to the UI. All database operations need to be run away from the main UI thread; you do so with coroutines and [`viewModelScope`](https://developer.android.com/topic/libraries/architecture/coroutines#viewmodelscope).
...
Notice that you did not use `viewModelScope.launch()` for `saveItem()` in the `ItemEntryViewModel.kt` file, but it is necessary for _`ItemEntryBody`_`()` when you call a repository method. You can only call suspend functions from a coroutine or another suspend function. The function `viewModel.saveItem()` is a suspend function.
